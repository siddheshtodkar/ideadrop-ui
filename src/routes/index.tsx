...
function HomePage() {
  ...
  const latestIdeas = ideas.splice(0, 3)
  return (
        ...
        <ul className="space-y-6">
          {latestIdeas.map(idea => (
            <li key={idea.id} className="border border-gray-300 rounded-lg shadow p-4 bg-white">
              ...
              <Link to='/ideas/$ideaId' params={{ ideaId: idea.id.toString() }} className="text-blue-600 hover:underline"> Read more → </Link>
            </li>
          ))}
        </ul>

        <div className="mt-6">
          <Link to="/ideas" className="w-full text-center inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-md transition">View All Ideas</Link>
        </div>
      </section>
    </div>
  )
